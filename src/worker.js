...
class WebSocketSession {
	...
	async handleConnect(text) {
		...
		if (firstData) {
			await this.remoteWriter.write(this.encoder.encode(firstData));
		}
	...
	async sendRemote(text) {
		if (this.remoteWriter) {
			try {
				await this.remoteWriter.write(this.encoder.encode(text));
			} catch (err) {
				this.sendError(err);
				this.close();
			}
...
export default {
	async fetch(request, env) {
		...
		new WebSocketSession(server, pathFallback, env);
