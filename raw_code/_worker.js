	...
	const pumpRemoteToWebSocket = async () => {
		try {
			while (!isClosed && remoteReader) {
				...
				if (done) break;
				if (webSocket.readyState !== webSocket.WS_READY_STATE_OPEN) break;
...
function safeCloseWebSocket(ws) {
	try {
		if (ws.readyState === webSocket.WS_READY_STATE_OPEN ||
			ws.readyState === webSocket.WS_READY_STATE_CLOSING) {
